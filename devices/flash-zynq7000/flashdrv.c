...
static ssize_t flashdrv_pageProgram(addr_t offs, const void *buff, size_t len, int dupa)
{
	ssize_t res;
	time_t timeout;
	const flash_cfi_t *cfi = &fdrv_common.info.cfi;
	...
	res = qspi_polledTransfer(buff, NULL, len, dupa ? -1 : TIMEOUT_CMD_MS);
	if (res < 0) {
		qspi_stop();
		return res;
	}
...
static int flashdrv_sync(unsigned int minor)
{
	...
	for (i = 0; i < pgNb; ++i) {
		...
		res = flashdrv_pageProgram(dst, src, pageSz, 0);
		if (res < 0) {
			return res;
		}
	}
