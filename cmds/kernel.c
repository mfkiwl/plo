...
static int cmd_kernel(int argc, char *argv[])
{
	...
	/* Read ELF header */
	res = phfs_read(handler, elfOffs, &hdr, sizeof(Elf64_Ehdr));
	if (res < 0) {
		log_error("\nCan't read %s, on %s", kname, argv[1]);
		return res;
	}
	...
	/* Read program segments */
	for (i = 0; i < hdr.e_phnum; i++) {
		elfOffs = hdr.e_phoff + i * sizeof(Elf64_Phdr);
		res = phfs_read(handler, elfOffs, &phdr, sizeof(Elf64_Phdr));
		if (res < 0) {
			log_error("\nCan't read %s, on %s", kname, argv[1]);
			return res;
		}

		if ((phdr.p_type == (Elf64_Word)PHT_LOAD)) {
			...
			/* Save kernel's beginning address */
			if (phdr.p_flags == (Elf64_Word)(PHF_R | PHF_X)) {
				kernelPAddr = entry->start;
			}
